...
===== A.{section}.{subsection}.{counter:typerule} Rule: validMinMaxStart
[[validMinMaxStart]]
// {vdm}
----
validMinMaxStart: FmiModelDescription * ModelVariables +> bool
validMinMaxStart(-, evs) == allOf
	([
		validMinMax(ev.min, ev.max, kindOf(ev)) =>
			ev.start <> nil =>
...
validMinMax: [real] * [real] * Kind +> bool
validMinMax(min, max, kind) ==
	let mk_(kmin, kmax) = minMaxKind(kind) in allOf
	([
		max <> nil =>
			-- @OnFail("%NAME: max %s is not a valid value of this type", max)
			( max <= kmax and max >= kmin ),

		min <> nil =>
			-- @OnFail("%NAME: min %s is not a valid value of this type", min)
			( min <= kmax and min >= kmin ),

		max <> nil and min <> nil =>
			-- @OnFail("%NAME: max %s not >= min %s", max, min)
			( max >= min )
	]);
----
// {vdm}
