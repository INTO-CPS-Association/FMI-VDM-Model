...
public class ClassMapper {
    ...
    /**
     * A class to define how to construct one destPackage class, passing srcPackage object fields to the Constructor and setter methods.
     */
    private static class MapParams {
        ...
        public MapParams(int lineNo, Class<?> srcClass, Class<?> destClass, List<Field> ctorFields, List<Field> setterFields, boolean unmapped) {
            this.lineNo = lineNo;
        ...
        public String toString() {
            return "map " + srcClass.getSimpleName() + " to " + destClass.getSimpleName() + (setterFields.isEmpty() ? "" : " set " + setterFields);
        }
    }
    ...
    /**
     * We cannot verify the constructors exist until all the mappings have been read, because we map the parameter types from the source as well. This
            ...
            if (mp.unmapped) {
                ...
                for (Field field : mp.setterFields) {
                    ...
                    try {
                        ...
                        error("No such setter: " + mp.destClass.getSimpleName() + ".set" + name + "(" + argType.getSimpleName() + ")");
                    }
                }
            }
        }
    ...
    /**
     * Private class to identify one class in the process of being converted, and a place to record objects/fields that need to be set when the
    ...
    /**
     * Return the load time of the mappings file. This is zeroed after the first request, because the mapping is not re-loaded after the first usage,
     * and so the cost is zero.
     */
